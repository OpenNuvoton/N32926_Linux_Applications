...
int main(int argc, char* argv[])
{
	...
	int fd, baud, loop, i, send_num, recv_num, offset;
	...
	while (1) {
		...
		recv_num = read_port(fd, buf, sizeof(buf), &tout);
		if (recv_num < 0)
			printf("read failed! (%s)\n", strerror(errno));
		else if (recv_num > 0) {
			printf("Read %d bytes\n", recv_num);
				//for (i = 0; i < recv_num; i++)
				//	printf("\tbuf[%d] = 0x%02X\n", i, buf[offset-recv_num+i]);

			offset = 0;
			while (offset < recv_num) {
				send_num = write_port(fd, buf+offset, recv_num-offset);
				if (send_num > 0) {
					offset += send_num;
